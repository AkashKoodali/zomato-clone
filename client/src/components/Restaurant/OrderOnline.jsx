...
const OrderOnline = () => {
  ...
  const restaurant = useSelector((globalData) => globalData.restaurant.selectedRestaurant);
  ...
  return (
    <>
      <div className="w-full flex items-start justify-start">
        <aside className="sticky top-16 hidden md:flex flex-col gap-1 border-r overflow-y-scroll border-gray-200 w-1/4">
          {menu.map((item, index) => (
            <MenuListContainer
              {...item}
              key={index}
        ...
        <div className="w-full px-6 md:w-3/4 overflow-auto top-0">
